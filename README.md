...
- **Reliability tab** in the dashboard that embeds the latest report inline — the evidence lives next to the feature, not buried in a repo
...
3. **Deterministic Scoring Engine** — the same Gaussian-decay + weighted categorical logic from VibeMatch 1.0. Every ranking decision happens here, not in the LLM.
...
- A free Google Gemini API key from [aistudio.google.com/apikey](https://aistudio.google.com/apikey). Free tier is **5 requests per minute** for `gemini-2.5-flash` — plenty for demo usage but worth knowing.
...
If `GEMINI_API_KEY` is missing, the natural-language input is gracefully disabled with an explanatory message, and the **Preset** / **Custom** profile sources in the sidebar continue to work. The dashboard never crashes on a missing key — this is one of the intentional reliability properties.
...
The model extracted the obvious cues (*slow*, *acoustic*) and made them extreme (`acousticness = 0.95`, `energy = 0.25`) while keeping `valence` positive — a relaxed *morning* vibe, not a sad one.
...
- **LLM rate limits deserve first-class design consideration.** Free-tier capacity planning matters even for demos — burst usage in the dashboard could easily trip a quota that a careful harness pacing strategy avoids.
...
| [src/profile_parser.py](src/profile_parser.py) | Gemini-backed natural-language → `UserProfile` parser with schema validation and clamping |
| [src/profiles.py](src/profiles.py) | 7 preset profiles + `KNOWN_GENRES` / `KNOWN_MOODS` / `KNOWN_MOOD_TAGS` derived from the catalog |
...
| [tests/](tests/) | 12 automated tests (scorer invariants + mocked parser tests) |
